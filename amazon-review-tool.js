...
(function () {
  ...
  function fetchPage(pageNum){
    ...
    return fetch(pageUrl,{credentials:'include'})
      ...
      .then(function(html){
        ...
        var totalOnPage=allEls.length;
        var newRevs=[], alreadySeenCount=0;
        allEls.forEach(function(el){
          ...
          if(isSeen(rid,body,date)){
            alreadySeenCount++;
            return;
          }
          markSeen({rid:rid,body:body,date:date});
          if(body||title) newRevs.push({rating:rating,title:title,body:body,date:date,vine:vine,rid:rid});
        });
        // ▼ 終了判定: ページの全件が既出 = Amazonが末尾を繰り返している
        var allDuplicate = totalOnPage > 0 && alreadySeenCount === totalOnPage;
        return {revs:newRevs, allDuplicate:allDuplicate, totalOnPage:totalOnPage};
      })
      .catch(function(e){ console.warn('fetchPage err p'+pageNum,e); return {revs:[],allDuplicate:false,totalOnPage:0}; });
  }

  // ── 1バッチ: 10ページ、全件重複が2回連続で終了 ──
  function fetchBatch(){
    var batchNew=[];
    var allDupCount=0; // 全件重複ページの連続回数
  ...
  window._bhlRender=function(){
    ...
    var topSection=isDone
      ...
      :'<div style="padding:10px 12px;background:#0F172A;border-radius:12px 12px 0 0;border-bottom:2px solid #1D4ED8">'
        +'<button id="_bhl_next_btn" onclick="window._bhlNext()" style="width:100%;padding:14px;background:#1D4ED8;border:none;color:#fff;border-radius:8px;font-size:14px;font-weight:700;cursor:pointer">'
