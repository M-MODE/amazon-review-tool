/* Amazon レビュー取得ツール v11.5 — bh life
 * ページ移動後に自動再実行（MutationObserver不使用・onload方式）
 * 商品ページ→レビューページ遷移→自動で全ページ収集
 */
(function () {
  'use strict';

  var NAV_KEY  = '_bhl_nav_';
  var AUTO_KEY = '_bhl_auto_';
  ...
  /* ── 商品ページ → レビューページへ遷移してから自動実行 ── */
  if (url.indexOf('/product-reviews/') < 0) {
    try { localStorage.setItem(autoKey, '1'); } catch(e) {}
  ...
  function showPanel(reviews, productTitle){
    ...
    var listHTML=reviews.slice(0,1000).map(function(r,i){
    ...
    panel.innerHTML=
      ...
      +'<button onclick="try{localStorage.removeItem(\''+stateKey+'\');localStorage.removeItem(\''+autoKey+'\');}catch(e){}document.getElementById(\'_bhl_amz_panel\').remove();alert(\'リセット済み\')" style="background:#1E293B;border:1px solid #334155;color:#94A3B8;font-size:11px;cursor:pointer;padding:4px 8px;border-radius:4px;margin-right:4px">🔄</button>'
      ...
      +'<div style="padding:10px 16px;background:#162032;border-top:1px solid #1E3A5F;border-radius:0 0 12px 12px;text-align:center;font-size:11px;color:#475569">'+(reviews.length>1000?'全'+reviews.length+'件中1000件表示（CSVで全件取得可）':'全'+reviews.length+'件を表示中')+'</div>';
  ...
  /* autoKeyフラグ確認（商品ページからの遷移 or 自動継続） */
  var isAuto = false;
  try { isAuto = !!localStorage.getItem(autoKey); localStorage.removeItem(autoKey); } catch(e) {}
  ...
  if(savedState && savedState.asin === asin && curPageInUrl > 1){
    ...
    /* 終了条件：次ページなし or 3ページ連続空 */
    if(!nextUrl || emptyPages >= 3){
