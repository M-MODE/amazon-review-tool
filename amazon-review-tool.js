...
(function () {
  ...
  function cleanDate(raw) {
    ...
    return raw.replace(/に日本でレビュー済み.*/,'').replace(/Reviewed in Japan on\s*/,'').trim();
  }
  ...
  function rKey(el,title,date){
    var id=el.id||''; if(id) return id;
    return (title+'|'+date).slice(0,80);
  }
  ...
  function parseReviews(doc){
    ...
    doc.querySelectorAll('[data-hook="review"]').forEach(function(el){
      ...
      if(key&&_seen[key]) return;
      if(key) _seen[key]=true;
  ...
  function getNextUrl(doc){
    ...
    for(var i=0;i<sel.length;i++){
      ...
      if(el){var h=el.getAttribute('href')||'';if(h&&h.indexOf('pageNumber')>=0) return h.startsWith('/')?'https://www.amazon.co.jp'+h:h;}
    }
    /* フォールバック：nextPageToken を含むリンク */
    var links=doc.querySelectorAll('a[href*="nextPageToken"]');
    if(links.length){var h=links[links.length-1].getAttribute('href')||'';return h.startsWith('/')?'https://www.amazon.co.jp'+h:h;}
    /* フォールバック２：現在より大きいpageNumberのリンク */
    var curM=(typeof doc.URL!=='undefined'?doc.URL:location.href).match(/pageNumber=(\d+)/);
    var cur=curM?parseInt(curM[1]):1;
    var best=null,bestN=cur;
    doc.querySelectorAll('a[href*="pageNumber="]').forEach(function(a){
      var m=(a.getAttribute('href')||'').match(/pageNumber=(\d+)/);
      if(m&&parseInt(m[1])>bestN){bestN=parseInt(m[1]);best=a.getAttribute('href');}
    });
  ...
  function upsertProg(page,count){
    ...
    el.innerHTML='<span style="color:#FF9900;font-weight:700">★ Amazonレビュー取得中</span>'
      ...
      +'<div style="flex:1;height:6px;background:#1E3A5F;border-radius:3px;overflow:hidden"><div style="height:100%;width:40%;background:linear-gradient(90deg,#FF9900,#FFD700);border-radius:3px;animation:_bhlp 1s ease-in-out infinite"></div></div>'
  ...
  window._bhlAmzCSV=function(){
    var vis=[];
    document.querySelectorAll('#_bhl_amz_list [data-idx]').forEach(function(el){if(el.style.display!=='none'){var idx=parseInt(el.getAttribute('data-idx'));if(window._bhlAmzAllReviews&&window._bhlAmzAllReviews[idx])vis.push(window._bhlAmzAllReviews[idx]);}});
  ...
  window._bhlAmzFilter=function(f,btn){
    ...
    document.querySelectorAll('#_bhl_amz_list [data-idx]').forEach(function(el){
      var rs=Math.round(parseFloat(el.getAttribute('data-r')||'0'));var v=el.getAttribute('data-vine')==='1';
  ...
  function showPanel(reviews,productTitle){
    ...
    var listHTML=reviews.slice(0,300).map(function(r,i){
      ...
      return '<div data-idx="'+i+'" data-r="'+r.rating+'" data-vine="'+(r.vine?'1':'0')+'" style="border-left:3px solid '+bc+';padding:10px 10px 10px 12px;margin-top:8px;background:#162032;border-radius:0 6px 6px 0">'
        +'<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:6px;margin-bottom:4px"><div><span style="color:#FF9900;font-size:13px;letter-spacing:1px">'+stars+'</span> <span style="color:#FF9900;font-size:11px;font-weight:700">'+(r.rating>0?r.rating.toFixed(1):'?')+'</span> '+lbl+'</div><span style="font-size:10px;color:#475569;white-space:nowrap;flex-shrink:0">'+r.date+'</span></div>'
  ...
  function fetchByPageNum(page){
    if(!window._bhlAmzRunning||page>totalPages+1){finish();return;}
    var u='https://www.amazon.co.jp/product-reviews/'+asin+'/?sortBy=recent&pageNumber='+page;
    fetch(u,{credentials:'include'})
      ...
      .then(function(html){
        ...
        allReviews=allReviews.concat(revs);
        upsertProg(pageCount,allReviews.length);
        if(revs.length>0){setTimeout(function(){fetchByPageNum(page+1);},300);}
        else{finish();}
      })
      .catch(function(){finish();});
  }
  ...
  /* nextPageToken付きリンクがあればそれを使用、なければページ番号方式 */
  if(firstNext){fetchByUrl(firstNext);}
  else{fetchByPageNum(2);}
