/* Amazon レビュー取得ツール v13.0 — bh life
 * fetch方式：ページ移動なし・バックグラウンド全件収集
 * 商品ページ→レビューページへ遷移のみ
 * レビューページでfetch全件取得
 */
(function(){
  ...
  if(window._bhlAmzRunning){
    if(confirm('取得中です。中断しますか？')){
      ...
      var pb=document.getElementById('_bhlProg');if(pb)pb.remove();
    }
    return;
  }
  ...
  function parseReviews(doc){
    ...
    doc.querySelectorAll('[data-hook="review"]').forEach(function(el){
      ...
      var rid=el.id||'';
      var key=rid||('bd:'+(body||'').slice(0,60)+'|'+date);
  ...
  /* 次ページURL：現在のページURLのpageNumberをインクリメント
     かつ li.a-last が disabled なら終了 */
  function getNextUrl(doc, curPage){
    /* li.a-last が disabled → 最終ページ */
    var lastLi=doc.querySelector('li.a-last,.a-pagination .a-last');
    if(lastLi && (lastLi.classList.contains('a-disabled')||!lastLi.querySelector('a'))){
      return null;
    }
    /* li.a-last の a タグから href を取得 */
    if(lastLi){
    ...
    var best=null,bestPg=curPage;
    doc.querySelectorAll('a[href*="nextPageToken"]').forEach(function(a){
  ...
  /* ── プログレスバー ── */
  function showProg(page,count){
    var el=document.getElementById('_bhlProg');
    if(!el){
      el=document.createElement('div');el.id='_bhlProg';
      el.style.cssText='position:fixed;top:0;left:0;right:0;z-index:2147483647;background:#0F172A;color:#fff;padding:12px 20px;font-family:sans-serif;font-size:14px;display:flex;align-items:center;gap:12px;box-shadow:0 2px 12px rgba(0,0,0,.6)';
      document.body.appendChild(el);
    }
    el.innerHTML='<span style="font-size:20px">★</span><div>'
      +'<div style="font-weight:700;color:#FF9900">レビュー取得中…</div>'
      +'<div style="font-size:12px;color:#94A3B8">累計 <strong style="color:#fff;font-size:16px">'+count+'</strong> 件 / ページ '+page+'　（このタブはそのままにしてください）</div>'
      +'</div>';
  }
  function removeProg(){var el=document.getElementById('_bhlProg');if(el)el.remove();}
  ...
  if(!p1next){
    ...
    setTimeout(function(){fetchPage(p1next,2);},600);
  }

  function fetchPage(fetchUrl,pg){
    if(!window._bhlAmzRunning)return;
    if(fetchUrl.startsWith('/'))fetchUrl='https://www.amazon.co.jp'+fetchUrl;
    fetch(fetchUrl,{credentials:'include'})
      ...
      .then(function(html){
        ...
        pageNum=pg;
        showProg(pg,allReviews.length);

        if(emptyCount>=3){finish();return;}

        var next=getNextUrl(doc,pg);
        if(!next){finish();return;}
        setTimeout(function(){fetchPage(next,pg+1);},800);
      })
      .catch(function(e){
  ...
  function showPanel(reviews,productTitle){
    ...
    panel.innerHTML=
      ...
      +'<div style="padding:10px 16px;background:#162032;border-top:1px solid #1E3A5F;border-radius:0 0 12px 12px;text-align:center;font-size:11px;color:#475569">'
        +(reviews.length>DISP?'全'+reviews.length+'件中'+DISP+'件表示（CSVで全件取得可）':'全'+reviews.length+'件を表示中')
      +'</div>';
